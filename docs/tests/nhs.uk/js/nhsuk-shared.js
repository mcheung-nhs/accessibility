...
function followLink(evt) {
  ...
  var target = targetAttr && !targetAttr.match(/^_(self|parent|top)$/i) ? targetAttr : false;
  if (target) {
    return false;
  }
...
Analytics.prototype.bindEvents = function bindEvents() {
  this.$body.on('click.analytics', '[data-' + this.attrName + ']', $.proxy(this._sendFromEvent, this));
};
...
Analytics.prototype._sendFromEvent = function _sendFromEvent(evt) {
  var params = this._getEventData(evt);
  var $el = $(evt.currentTarget);
  var callback = void 0;

  /*
   * TODO: What if we are trying to track clicks on an anchor link?
   * The page will reload before the tracking Ajax has time to complete.
   * Previously we artificially delayed the navigation while the tracking
   * event was sent. That is a bad approach
...
Analytics.prototype._getEventData = function _getEventData(e) {
  ...
  var component = $el.data('analytics');
  var componentType = $el.data('analytics-type');
  var params = [];
  var name = void 0;
  var value = void 0;
...
Analytics.prototype._gaExists = function () {
  return _typeof(window.ga) === (typeof Function === 'undefined' ? 'undefined' : _typeof(Function));
};

Analytics.prototype._wtExists = function () {
...
;$( document ).ready(function() {
  var elements = $('.sidebar-navigation--sticky');
...
(function ($) {
  ...
  function init() {
    ...
    $('.c-search-menu').attr('id', 'q_listbox');
  }
...
;(function($) {
// ----------------------
// Banner code, for emergency alerts, surveys and invitations
// ----------------------

var surveyBannerStr = "<div id='BannerDisplay' class='survey-banner-wrap clear' runat='server'>" +
    "<div id='SurveyBanner' class='survey-banner-content' runat='server'>" +
    "<div class='survey-invite'>" +
    "<p><span>[Invitation1]</span></p>" +
    "<p>[Invitation2]</p>" +
    "</div>" +
    "<ul>" +
    "<li class='leave-feedback agree'><a id='AgreeButton' runat='server' onclick='dcsMultiTrack(&quot;DCSext.OnSiteSurvey2&quot;,&quot;Yes&quot;,&quot;WT.dl&quot;,&quot;121&quot;)'>[Agree]</a></li>" +
    "<li class='go-back postpone' id='PostponeButton'><a runat='server' onclick='dcsMultiTrack(&quot;DCSext.OnSiteSurvey2&quot;,&quot;Maybe Later&quot;,&quot;WT.dl&quot;,&quot;121&quot;)'>[Postpone]</a></li>" +
    "<li class='go-back disagree'><a id='DisagreeButton' runat='server' onclick='dcsMultiTrack(&quot;DCSext.OnSiteSurvey2&quot;,&quot;No&quot;,&quot;WT.dl&quot;,&quot;121&quot;)'>[Disagree]</a></li>" +
    "</ul>" +
    "</div>" +
    "</div>";

var alphaBannerStr = "<div id='BannerDisplay' class='alpha-banner-wrap clear' runat='server'>" +
    "<div id='AlphaBanner' class='alpha-visit-content' runat='server'>" +
    "<div class='survey-invite'>" +
    "<p><span>[Invitation1]</span></p>" +
    "<p>[Invitation2]</p>" +
    "</div>" +
    "<ul>" +
    "<li class='leave-feedback agree'><a id='AgreeButton' class='agree' runat='server' onclick='dcsMultiTrack(&quot;DCSext.BetaInvite&quot;,&quot;Yes&quot;,&quot;WT.dl&quot;,&quot;121&quot;)'>[Agree]</a></li>" +
    "<li class='go-back postpone'><a id='PostponeButton' class='postpone' runat='server' onclick='dcsMultiTrack(&quot;DCSext.BetaInvite&quot;,&quot;Maybe Later&quot;,&quot;WT.dl&quot;,&quot;121&quot;)'>[Postpone]</a></li>" +
    "<li class='go-back disagree'><a id='DisagreeButton' class='disagree' runat='server' onclick='dcsMultiTrack(&quot;DCSext.BetaInvite&quot;,&quot;No&quot;,&quot;WT.dl&quot;,&quot;121&quot;)'>[Disagree]</a></li>" +
    "</ul>" +
    "</div>" +
    "</div>";

var alertBannerStr = "<div class='banner'>" +
    "[Content]" +
    "<p class='updated'>Last update:<br />[CreatedDate]</p>" +
    "<span id='closealert'>Close</span>" +
    "</div>";

var dialogText = "<h1 class='widget-header'>You are now visiting an external website</h1><span class='widget-content'><p>We are not responsible for, endorse, guarantee or monitor the content on other websites. We recommend that you review the privacy and security policy of other websites, as their policies may differ from our own.</p>" +
    "<p>Click 'Return' to return to the page on NHS.UK that you were just viewing. Or click 'Continue' to be taken to the external website.</p></span>";
...
$(document).ready(function () {
    //hide alert button if js turned on
    $('#alertBtn').hide();
    var url = isTestMode() ?
    ...
    $.ajax({
        ...
        success: function (response) {
            ProcessBanners(response);
        }
    });
});

function ProcessBanners(bannersArray) {

    var foundBanner = false;
    var banner;
    for (var key = 0; key < bannersArray.length; key++) {
        banner = bannersArray[key];
        if (cookieCheck(banner)) {
            continue;
        };
        if (banner.Style === "Alert") {
            foundBanner = true;
            break;
        } else if (isMatchedUrl(banner)) {
            foundBanner = true;
            break;
        }
    }
    if (foundBanner) {
        var $bannerEl = formatBannerAsElement(banner);
        setButtonVisibility(banner, $bannerEl);
        insertBanner(banner, $bannerEl);
        setupHandlers(banner);
    }
}

function cookieCheck(banner) {
    return ($.cookie("Banner" + banner.BannerId) === banner.Title);
}

function isMatchedUrl(banner) {
    var url = window.location.href.toLowerCase();
    if (banner.Excluding.length > 0) {
        var excludeFroms = banner.Excluding.toLowerCase().split(";");
        for (var i = 0; i < excludeFroms.length; i++) {
            if ((excludeFroms[i].length > 0) && (url.indexOf(excludeFroms[i]) > -1)) {
                return false;
            }
        }
    }
    if (banner.Including.length === 0) {
        return true;
    }
    var includeIns = banner.Including.toLowerCase().split(";");
    for (var j = 0; j < includeIns.length; j++) {
        if ((includeIns[j].length > 0) && (url.indexOf(includeIns[j]) > -1)) {
            return true;
        }
    }
    return false;
...
function formatBannerAsElement(banner) {
    var bannerTemplate;
    switch (banner.Style) {
        case "Alpha":
            bannerTemplate = alphaBannerStr;
            break;
        case "Alert":
            bannerTemplate = alertBannerStr;
            break;
        default:
            bannerTemplate = surveyBannerStr;
    }
    return $(instantiateBannerTemplate(bannerTemplate, banner));
}

function instantiateBannerTemplate(template, banner) {
    return template.replace(/\[(\w*)\]/g, function (m, key) { return banner.hasOwnProperty(key) ? banner[key] : ""; });
}

function setButtonVisibility(banner, $bannerEl) {
    if (!banner.Agree) {
        $("#AgreeButton", $bannerEl).remove();
    }
    if (!banner.Postpone) {
        $("#PostponeButton", $bannerEl).remove();
    }
    if (!banner.Disagree) {
        $("#DisagreeButton", $bannerEl).remove();
    }
}

function insertBanner(banner, $bannerEl) {
    if (banner.Style === "Alert") {
        $("nav.c-nav-primary").after($bannerEl);
        $(".wrap").css("background-position", "center " + ($('.banner').outerHeight() + 3) + "px")
        if ($('#cookie-banner').length) {
            $("body").css("background-position", "center " + ($('.banner').outerHeight() + 57) + "px");
        }
        else {
            $("body").css("background-position", "center " + ($('.banner').outerHeight() + 8) + "px");
        }
    }
    else {
        $($bannerEl).prependTo("body");
    }
}

function setupHandlers(banner) {

    $("#PostponeButton, #DisagreeButton, #AgreeButton, #closealert").hover(function () {
        this.style.cursor = "pointer";
    });
    $("#PostponeButton").on("click", banner, function (event) {
        var millisecondDelay = ((((event.data.DelayDays * 24) + event.data.DelayHours) * 60) + event.data.DelayMinutes) * 60 * 1000;
        var expiryDate = new Date(Date.now() + millisecondDelay);
        $(".survey-banner-wrap, .alpha-banner-wrap").hide();
        $.cookie("Banner" + event.data.BannerId, event.data.Title, { expires: expiryDate, path: "/" });
    });
    $("#DisagreeButton").on("click", banner, function (event) {
        $(".survey-banner-wrap, .alpha-banner-wrap").hide();
        $.cookie("Banner" + event.data.BannerId, event.data.Title, { expires: 100, path: "/" });
    });
    $("#AgreeButton").on("click", banner, function (event) {
        $(".survey-banner-wrap, .alpha-banner-wrap").hide();
        $.cookie("Banner" + event.data.BannerId, event.data.Title, { expires: 100, path: "/" });
        var win = window.open(event.data.BannerUrl, "_blank");
        win.focus();
    });
    $("#closealert").on("click", banner, function (event) {
        $(".banner").hide();
        $(".wrap").css("background-position", "center top");
        if ($('#cookie-banner').is(":visible")) {
            $("body").css("background-position", "center 53px");
        }
        else {
            $("body").css("background-position", "center 5px");
        }
        $.cookie("Banner" + event.data.BannerId, event.data.Title, { expires: 1, path: "/" });
    });
}
...
;$(document).ready(function() {
  ...
  if(msie <= 9 && $(window).width() > 479) {
    $('ol.twocolumns').attr('style','list-style: none');
    $('.twocolumns').multicolumn();
  }
});
    ...
    doColumns : function($el) {
      ...
      $vel.each(function() {
        ...
        for (var i = 0; i < columnCount; i++) {
          ...
          // Calculate and insert the list item numbers
          $.each( $listItems, function( key, value ) {
            var a_text = count + ". " + $(value).text();
            if($(value).find('a').text() != ''){
              $(value).find('a').text(a_text);
            } else {
              $(value).text(a_text);
            }
            count = count + 1;
...
;$(document).ready(function () {
 ...
 $seenBannerCookie = $.cookie(cookieName);
    ...
    var closeBanner = function () {
        cookieBannerHeight = $('#' + bannerId).height();
        $('#' + bannerId).slideUp();
        $('body').animate({ 'background-position-y': '-=' + cookieBannerHeight + 'px' }, 400, 'swing');
        dcsMultiTrack("DCSext.CookieBanner", "Dismiss", "WT.dl", "121");
    };

    var htmlContent = '<div id="' + bannerId + '"><div class="cookie-text clear"><p>The NHS website uses cookies to improve your on-site experience. <a class="' + policyCls + '" href="http://www.nhs.uk/aboutNHSChoices/aboutnhschoices/termsandconditions/Pages/cookies-policy.aspx" target="_blank">Find out more about cookies</a></p><p class="' + bannerCloseCls + '"><a href="#"><img src="img/icon-close-das.gif" alt="Close cookie banner" /></a></p></div></div>';

    if (!$seenBannerCookie) {
        $.cookie(cookieName, true, { expires: 90, path: '/' });
        $('body').prepend(htmlContent).css('background-position', 'left 53px');
  ...
  var render = function() {
    ...
    */
    if( window.history.pushState ) {
      ...
      if( $openItem ) {
        var tabname = $openItem.find('> a').data('tabname')
      ...
      // Add a preceding `?` only if we have query params
      newQueryString = newQueryString ? '?' + newQueryString : '';
      // '.' = stay on this path
      var newURL = '.' + newQueryString + window.location.hash;
      window.history.replaceState(state.openIndex, $('title').text(), newURL);
    }
...
;window.addEventListener("load", function() {
    ...
    if (form) {
        ...
        var radioButtons = document.getElementsByClassName("statusRadio");
...
(function($) {
  ...
  // If there is no livewell-nav element, don't activate any of this code
  if ( $('.livewell-nav').length == 0 ) {
    return;
  }

  var $items = $('.livewell-nav__item')
  var $spacer = $('.livewell-nav__spacer')
  ...
  */
  var state = (function() {
    var initialOpenIndex = null
    $items.each(function(index, element) {
      if( $(element).hasClass('livewell-nav__item--selected') ) {
        initialOpenIndex = index
      }
    })
  ...
  */
  var render = function() {
    $items.each(function(i, item) {
      var $item = $(item)
      if (state.openIndex === i ) {
        $item.addClass('livewell-nav__item--selected')
      } else {
        $item.removeClass('livewell-nav__item--selected')
      }

      $item.find('.livewell-nav-subnav__item').each(function(j) {
        if (state.openSubTabs[i][j]) {
          $(this).addClass('livewell-nav-subnav__item--open')
        } else {
          $(this).removeClass('livewell-nav-subnav__item--open')
        }
      })
    })
    ...
    */
    ...
    if( $openItem ) {
      //measure the navigation height, if open.
      var $openSubnav = $openItem.find('.livewell-nav__contents')
      height = $openSubnav.height() + 30
    }
    $spacer.css('height', height)
...
;window.addEventListener("load", function() {
    ...
    var form = document.getElementById("ds_weightlosssignup");
    ...
    if (form) {
        ...
        form.addEventListener("submit", function(event) {
            ...
            var checks = [
                ...
                ["cd_LASTNAME", "ds_lastnameError", "Please enter your last name", "ds_lastnameGroup"],
                ["Email", "ds_emailError", "Please enter a valid email address", "ds_emailGroup"],
                ["cd_GENDER", "ds_genderError", "Please indicate your gender", "ds_genderGroup"],
                ["cd_LTCOR1KGPLUS", "ds_relevanceError", "Please choose one of the following options", "ds_relevanceGroup"],
                ["agree_terms", "ds_tcError", "Please confirm you agree to our terms of service", "ds_confirmGroup"],
                ["agree_terms_2", "ds_tcError_2", "Please confirm you agree to our terms of service", "ds_confirmGroup_2"],
            ];
            ...
            // Special error message when Email is blank
            var email = document.getElementById('ds_email');
            if (email.value === '') {
              emailError = document.getElementById('ds_emailError');
        ...
        function checkInput(inputName, spanId, errorMsg, divId) {
            ...
            var input = document.getElementsByName(inputName)[0];
            var hasHtml5Validation = !! input.validity

            var inputValid = true;
            if(hasHtml5Validation && !input.validity.valid) {
              inputValid = false;
              ...
              if((input.type === "text" && input.value == "") ||
                ...
                var radioButtons = document.getElementsByName(inputName);
            ...
            if(!inputValid) {
              errorSpan.innerHTML = errorMsg;
              errorSpan.setAttribute("style", "display: list-item; list-style-position: outside; margin-bottom: 16px;");
