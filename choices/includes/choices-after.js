...
function fRemoveValue(e) {
  ...
  if (!($(oTarget).hasClass('excludeInputClear'))) {
    ...
    var leaveCommentTextAreaId = "ctl00_ctl00_ctl00_PlaceHolderMain_column1Content_ctl00_reviewText"; 
    if ((oTarget.value == oTarget.defaultValue) && !ignoreSite  ) {
      oTarget.value = '';
    }
    else if(oTarget.id == leaveCommentTextAreaId){ //extra check for new formatted default text on leave review pages
      oTarget.value = '';
    }
  }
}
      ...
      switch (e.which) {
        ...
        case 40: // down
          if ($(this).closest('li').is(':last-child')) {
            $(this).closest('ul').find('a').first().focus();
          } else {
            $(this).closest('li').next('li').find('a').focus(); // what if the next li happens to not have an anchor tag?
          }
          break;
        ...
        case 38: // up      
          if ($(this).closest('li').is(':first-child')) {
            console.log('first');
            $(this).closest('li').closest('ul').find('li:last-child a').focus();
          } else {
